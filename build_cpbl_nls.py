...
"""
build_cpbl_nls.py
Compiles Windows NLS files directly into the unified 36-byte header CPBL specification.
"""
...
def parse_nls(nls_path):
    with open(nls_path, "rb") as f:
        ...
        if max_char_size == 1:
            ...
            for mb, uc in sbcs_map.items():
                if uc != 0 and uc != uni_def_char: wc2mb_map[uc] = mb
    ...
    return code_page, sbcs_map, dbcs_map, dbcs_first_bytes, wc2mb_map
...
def build_unified_blob(nls_path):
    ...
    # Map Lead Boundary Windows
    for i in range(256):
        if i in sbcs_map:
            dbcs_lead_table[i] = sbcs_map[i] & 0x7FFF
        elif i in dbcs_first_bytes:
            dbcs_lead_table[i] = 0x8000
    ...
    # Compile the compressed 2-tier reverse directory
    ...
    # Calculate absolute offsets inside asset package
    off_windows = base_header_size + 512
